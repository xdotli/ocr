...
type UploadStatus = 'idle' | 'uploading' | 'uploaded' | 'processing' | 'processed' | 'error'
...
export function OcrComparisonTool() {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files) {
      ...
      setOcrResults(prevResults => [...prevResults, ...newFiles.map(() => ({ ...initialOCRResults }))])
  ...
  const mockOCRRequest = (provider: keyof OCRResults, content: string): Promise<OCRResult> => {
    return new Promise((resolve) => {
  ...
  const processOCR = async () => {
    ...
    try {
      const newOcrResults = await Promise.all(files.map(async (file, fileIndex) => {
        const content = file.content.join('\n\n')
        const results: OCRResults = { ...initialOCRResults }

        for (const provider of Object.keys(initialOCRResults) as Array<keyof OCRResults>) {
      ...
      setSummaryStats({
        tokensUsed: files.reduce((acc, file) => acc + file.content.join(' ').split(' ').length, 0),
        totalCost: (files.reduce((acc, file) => acc + file.content.join(' ').split(' ').length, 0) * 0.0001).toFixed(2),
        latency: Math.max(...newOcrResults.flatMap(result => Object.values(result).map(r => r.time || 0)))
      })
        ...
        <div className="w-1/2 flex flex-col gap-4">
          ...
          <Card className="flex-grow">
            ...
            <CardContent className="h-[calc(100vh-400px)]">
              {files.length > 0 ? (
                <iframe src={files[currentFileIndex].url} className="w-full h-full" />
              ) : (
              ...
              <ScrollArea className="h-40 w-full">
                <div className="flex gap-4">
                  {files.map((file, index) => (
                    <div 
                      key={index} 
                      className={`flex-shrink-0 w-32 border rounded p-2 flex flex-col items-center cursor-pointer ${
                        currentFileIndex === index ? 'border-primary' : 'border-gray-200'
                      }`}
                      onClick={() => setCurrentFileIndex(index)}
                    >
                      <div className="w-full h-20 bg-gray-200 flex items-center justify-center mb-2 overflow-hidden">
        ...
        <div className="w-1/2 flex flex-col gap-4">
          <Card className="flex-grow">
            <CardContent className="p-0">
              <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as keyof OCRResults)} className="flex flex-col h-full">
                  ...
                  <TabsList className="w-full bg-transparent">
                    {Object.entries(initialOCRResults).map(([key]) => (
                      <TabsTrigger 
                        key={key} 
                        value={key} 
                        className="flex-1 data-[state=active]:bg-white data-[state=active]:shadow-none rounded-t-md px-1 py-1"
                      >
                        <div className="flex flex-col items-center">
                          <span className="font-semibold">{key}</span>
                          {currentResults[key as keyof OCRResults].time !== null && (
                            <span className="text-xs text-muted-foreground">
                              ({currentResults[key as keyof OCRResults].time?.toFixed(2)}s)
                            </span>
                          )}
                        </div>
                      </TabsTrigger>
                ...
                {Object.entries(initialOCRResults).map(([key]) => (
                  <TabsContent key={key} value={key} className="flex-grow mt-0 border-x border-b rounded-b-lg bg-white">
                    <ScrollArea className="h-[calc(100vh-18rem)]">
                      {files.length === 0 ? (
                        <p className="text-gray-500 p-4">No file uploaded</p>
                      ) : uploadStatus === 'processing' ? (
                        ...
                        <p className="text-gray-500 p-4">OCR not started</p>
                      )}
                    </ScrollArea>
                  </TabsContent>
                ))}
